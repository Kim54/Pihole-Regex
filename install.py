...
url_regexps_remote = 'https://raw.githubusercontent.com/Kim54/Pihole-Regex/master/Regex'
install_comment = 'Kim54/Regex'
...
regexps_remote = set()
regexps_local = set()
regexps_Kim54_local = set()
regexps_legacy_Kim54 = set()
regexps_remove = set()
...
path_legacy_Kim54_regex = os.path.join(path_pihole, 'Kim54-regex.list')
...
if db_exists:
    ...
    # Fetch all current Kim54 regexps in the local db
    c.execute('SELECT domain FROM domainlist WHERE type = 3 AND comment = ?', (install_comment,))
    regexps_Kim54_local_results = c.fetchall()
    regexps_Kim54_local.update([x[0] for x in regexps_Kim54_local_results])
    ...
    regexps_remove = regexps_Kim54_local.difference(regexps_remote)
    ...
    # Delete Kim54-regex.list as if we've migrated to the db, it's no longer needed
    if os.path.exists(path_legacy_Kim54_regex):
        os.remove(path_legacy_Kim54_regex)
    ...
    if regexps_local:
        ...
        if os.path.isfile(path_legacy_Kim54_regex) and os.path.getsize(path_legacy_Kim54_regex) > 0:
            print('[i] Existing Kim54-regex install identified')
            # Read the previously installed regexps to a set
            with open(path_legacy_Kim54_regex, 'r') as fOpen:
                regexps_legacy_Kim54.update(x for x in map(str.strip, fOpen) if x and x[:1] != '#')

                if regexps_legacy_Kim54:
                    print('[i] Removing previously installed regexps')
                    regexps_local.difference_update(regexps_legacy_Kim54)
    ...
    # Output Kim54 remote regexps to Kim54-regex.list
    # for future install / uninstall
    with open(path_legacy_Kim54_regex, 'w') as fWrite:
